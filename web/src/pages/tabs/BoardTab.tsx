...
import {
  ...
  Circle, Columns3, GripVertical, Inbox, List, MoreHorizontal, Play, Plus, Search, X
} from 'lucide-react'
...
export function BoardTab() {
  ...
  // Synced issues (Linear/Jira)
  const { data: issuesData } = useApi(() => api.issues(projectId).catch(() => ({ issues: [] })), [projectId])
  const syncedIssues = (issuesData as any)?.issues || []
      ...
      <div className="flex-1 flex flex-col min-h-0">
        ...
        {syncedIssues.length > 0 && (
          <div className="shrink-0 border-t border-border px-5 py-3">
            <p className="text-micro font-medium text-muted-foreground uppercase tracking-wider mb-2">Synced Issues</p>
            <div className="space-y-1.5 max-h-[200px] overflow-y-auto">
              {syncedIssues.slice(0, 20).map((issue: any, i: number) => (
                <div key={issue.id || i} className="rounded-lg border border-border bg-card px-3.5 py-2.5 transition-colors hover:border-foreground/15">
                  <div className="flex items-center gap-2 text-xs">
                    {issue.identifier && <span className="text-muted-foreground/50 tabular-nums shrink-0">{issue.identifier}</span>}
                    <span className="text-[13px] text-foreground/90 truncate flex-1">{issue.title || issue.description || issue.id}</span>
                    {issue.state?.name && <span className="text-micro text-muted-foreground bg-surface-2 rounded-full px-2 py-0.5 shrink-0">{issue.state.name}</span>}
                  </div>
                </div>
              ))}
            </div>
...
function KanbanView({ tasks, projectId, refresh, navigate, hasFilter, sortBy, sortDir, manualOrder }: {
  ...
  return (
    <div className="flex-1 flex overflow-x-auto min-h-0 px-3 pt-2 pb-3 gap-2">
...
function KanbanColumn({ colKey, label, items, isOver, draggingId, hasFilter, projectId, onDragOver, onDragLeave, onDrop, onDragStart, onDragEnd, onClickTask }: {
  ...
  return (
    <div
      className={cn("flex-1 min-w-[300px] max-w-[420px] flex flex-col rounded-lg", isOver && "ring-1 ring-foreground/10")}
      onDragOver={e => { e.preventDefault(); onDragOver() }} onDragLeave={onDragLeave} onDrop={onDrop}
    >
      {/* Column header */}
      <div className="flex items-center gap-2 px-3 py-2 text-[13px] shrink-0">
        ...
        <div className="ml-auto flex items-center gap-0.5">
          <button type="button" className="h-5 w-5 flex items-center justify-center rounded text-muted-foreground/25 hover:text-muted-foreground hover:bg-surface-2 transition-colors">
      ...
      <div className="flex-1 overflow-y-auto px-1.5 pb-1.5 space-y-1.5">
        {shown.map(t => (
          <div key={t.id} draggable
            ...
            className={cn(
              "group rounded-lg border border-border bg-card px-3.5 py-3 cursor-pointer transition-colors hover:border-foreground/15",
          ...
          >
            ...
            <div className="flex items-center justify-between mb-1">
              {t.id.length < 20
                ? <span className="text-[11px] text-muted-foreground/50 tabular-nums">{t.id}</span>
                : <span />}
              <span className="h-5 w-5 rounded-full bg-surface-2 shrink-0" />
            </div>
            {/* Row 2: Status + title */}
            <div className="flex items-start gap-2">
              <StatusIcon section={t.completed ? 'previously_active' : t.section} />
              <p className="text-[13px] leading-snug text-foreground/90 line-clamp-3 flex-1">{t.description}</p>
            </div>
            ...
            {/* Row 4: Date */}
            <p className="text-[11px] text-muted-foreground/40 mt-1.5">Created {formatDate(t.createdAt)}</p>
          </div>
        ))}
