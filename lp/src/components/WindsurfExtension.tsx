 ...
 return (
  <section ref={containerRef} className="windsurf-extension-section py-24 px-6 relative overflow-hidden">
   ...
   <div className="absolute inset-0 -z-10">
    <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-cat-mauve/10 rounded-full blur-3xl" />
    <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-cat-sapphire/10 rounded-full blur-3xl" />
   </div>
   ...
   >
    ...
    >
     ...
     <div className="flex items-center justify-center gap-3 mb-6">
      <motion.div
       ...
       className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gradient-to-r from-purple-500/20 to-blue-500/20 backdrop-blur-sm border border-cat-mauve/30"
      >
       <Sparkles className="w-4 h-4 text-cat-mauve" />
      ...
      <motion.div
       ...
       className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-cat-mauve/10 border border-cat-mauve/20"
      >
       <Clock className="w-3 h-3 text-cat-mauve animate-pulse" />
       <span className="text-xs font-medium text-cat-mauve">
        In Validation
       </span>
      </motion.div>
     </div>
    ...
    >
     {features.map((feature, index) => (
      <motion.div
       ...
       className="group relative p-6 rounded-2xl bg-card/50 backdrop-blur-sm border border-border/50 hover:border-cat-mauve/50 transition-all duration-300"
      >
    ...
    <motion.div
     ...
     transition={{ duration: 0.8 }}
     viewport={{ once: true }}
     className="mb-20 max-w-4xl mx-auto py-12 px-6 bg-gradient-to-br from-purple-500/5 to-blue-500/5 rounded-2xl border border-cat-mauve/20"
    >
      ...
      {timeline.map((item, index) => (
       ...
       >
        <div className="flex items-center gap-6">
         ...
         <div className="relative">
          <div className={`w-4 h-4 rounded-full border-2 ${
           item.status === 'completed'
            ...
            : item.status === 'active'
            ? 'bg-cat-mauve border-cat-mauve shadow-lg shadow-purple-500/50'
            : 'bg-background border-muted-foreground/30'
          }`}>
           {item.status === 'active' && (
            <div className="absolute inset-0 rounded-full bg-cat-mauve animate-ping" />
           )}
          </div>
          {/* Vertical Line */}
         ...
         {/* Content */}
         <div className="flex-1 p-4 bg-card/50 rounded-xl border border-border/50 hover:border-cat-mauve/30 transition-all duration-300">
          <div className="flex items-center justify-between">
           <div>
            <h3 className={`text-lg font-semibold mb-1 ${
             item.status === 'completed' ? 'text-muted-foreground line-through' :
             item.status === 'active' ? 'text-cat-mauve' : 'text-foreground'
            }`}>
             {item.phase}
            </h3>
           ...
           <div className="text-right">
            ...
            {item.status === 'active' && (
             <div className="mt-2 px-2 py-1 bg-cat-mauve/20 text-cat-mauve rounded-full text-xs inline-block">
              Active
             </div>
            )}
           </div>
