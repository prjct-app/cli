...
const DIR_TYPE_PATTERNS: { type: DirectoryEntry['type']; patterns: RegExp[] }[] = [
  ...
  { type: 'source', patterns: [/^src$/i, /^lib$/i, /^core$/i, /^app$/i, /^pages$/i, /^components$/i, /^services$/i, /^utils$/i] },
...
const PATTERN_DETECTORS: { name: string; detect: (index: ProjectIndex) => number; evidence: (index: ProjectIndex) => string[] }[] = [
  {
    name: 'monorepo',
    detect: (idx) => {
      const hasWorkspaces = idx.configFiles.some(cf =>
        cf.type === 'package.json' && cf.parsed?.workspaces
      )
      const hasPackages = idx.directories.some(d =>
        d.path === 'packages' || d.path === 'apps'
      )
  ...
  },
  {
    name: 'api-first',
    detect: (idx) => {
      const hasApiDir = idx.directories.some(d =>
        d.path.includes('api') || d.path.includes('routes')
      )
      const hasOpenApi = idx.configFiles.some(cf =>
        cf.path.includes('openapi') || cf.path.includes('swagger')
      )
      return hasOpenApi ? 0.9 : hasApiDir ? 0.6 : 0
    },
    evidence: (idx) => idx.directories
  ...
  },
  {
    ...
    detect: (idx) => {
      const hasComponents = idx.directories.some(d =>
    ...
    evidence: (idx) => idx.directories
      .filter(d => d.path.includes('components'))
      .map(d => d.path + '/'),
  },
  {
    name: 'serverless',
    detect: (idx) => {
      const hasServerless = idx.configFiles.some(cf =>
        cf.path.includes('serverless') || cf.path.includes('netlify') || cf.path.includes('vercel')
      )
      const hasLambda = idx.directories.some(d =>
        d.path.includes('functions') || d.path.includes('lambda')
      )
      return hasServerless ? 0.9 : hasLambda ? 0.6 : 0
    },
    evidence: (idx) => idx.configFiles
...
export class ProjectIndexer {
  ...
  async incrementalUpdate(changedPaths?: string[]): Promise<ScanResult> {
    ...
    // Load existing index
    let index = await indexStorage.readIndex(this.projectId)
    if (!index) {
    ...
    if (changedPaths && changedPaths.length > 0) {
      ...
      if (changes.deleted.length > 0) {
        index.relevantFiles = index.relevantFiles.filter(
  ...
  async getRelevantContext(maxTokens: number = 50000): Promise<RelevantContext> {
    ...
    const compressionRate = originalTokens > 0
  ...
  private async scanAllFiles(options: IndexOptions = {}): Promise<Map<string, FileStats>> {
    ...
    try {
      const excludeDirs = Array.from(IGNORE_DIRS).map(d => `-not -path "*/${d}/*"`).join(' ')
      const extensions = Array.from(SOURCE_EXTENSIONS).map(e => `-name "*${e}"`).join(' -o ')
  ...
  private async scanFiles(paths: string[]): Promise<Map<string, FileStats>> {
    ...
    const results = await Promise.all(
      paths.map(p => this.getFileStats(p))
    )
  ...
  private async detectStack(configFiles: ConfigFileEntry[]): Promise<DetectedStack> {
    ...
    if (packageJson?.parsed) {
      ...
      const deps = {
        ...(packageJson.parsed as Record<string, unknown>).dependencies as Record<string, string> || {},
        ...(packageJson.parsed as Record<string, unknown>).devDependencies as Record<string, string> || {},
      }
    ...
    // Docker & CI
    stack.hasDocker = configFiles.some(cf =>
      cf.type === 'Dockerfile' || cf.type.includes('docker-compose')
    )
