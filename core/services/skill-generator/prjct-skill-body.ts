...
export function buildPrjctSkillBody(ctx: SkillContext): string {
  return [
    ...
    '- **Client never tutors basics.** Do not ask the user to explain language semantics, project patterns, or code already on screen. Read the file; query `prjct search` / `guard` / `context memory` first. If `let x` is followed by `x++`/`+=`, `let` is correct — never flag prefer-const or ask "why let?". That is a hallucination, not a finding.',
    '',
    ...
    '- **Review FPs burn client trust:** style nits without reading mutations, inventing bugs, or quizzing the user on code they can already see. Precision > recall. Use `prjct judgment` so evidence tax + mechanical refute kill noise.',
    '',
  ].join('\n')
}
...
export function buildPrjctSkillReference(): string {
  return [
    ...
    '',
    '**Anti-hallucination (client trust):** Never quiz the user about code on screen. Trace the full binding before prefer-const (`let x` + `x++` is correct). No style nits without file:line + proof. Prefer silence over a false finding — clients must not tutor the agent.',
    '',
    ...
    '4. **Never invent defects or ask the human to explain language basics** visible in the diff. If unsure, re-read the file — do not escalate confusion to the client.',
    '',
