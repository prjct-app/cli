...
export function buildPrjctSkillReference(): string {
  return [
    ...
    '',
    'Modes (pick one): `expansion` (adversarial — "what could break / is missing"), `polish` (final pass on correct code), `triage` (fast pass, auto-fix only the obvious).',
    '',
    'Methodology:',
    '1. **Dispatch as subagent** when the diff touches >5 files (see "Subagent dispatch"). The subagent reads the diff + memory in a fresh context and returns a finding list.',
    ...
    '4. Auto-fix only the OBVIOUS (typos, wrong var names, missing await on a discarded promise). Anything ambiguous → flag, do not touch.',
    '5. Stop conditions: max 3 auto-fixes per file; never refactor outside the diff scope.',
    ...
    '4. Stop conditions: max 3 failed fixes per bug — escalate to a human with details.',
    ...
    '',
    'Methodology:',
    '1. **Dispatch as subagent** for any review touching authentication, payment, file I/O, shell exec, or DB queries. Security review is read-heavy — context rot here costs more than elsewhere.',
    ...
    '4. Confidence gate: only report findings rated 8/10+ on exploit feasibility AND impact. Below = appendix only.',
    '5. False-positive exclusions: skip CSRF on idempotent GET, SQL injection on parameterized queries, XSS on already-escaped templates, leaks on logged-error-codes-without-PII. Capture exclusions as `prjct remember decision`.',
    ...
    '',
    'Methodology:',
    '1. **Dispatch the trace+hypothesis phase as a subagent** when the bug spans more than one module. Subagent reads logs, source, recent diffs in fresh context and returns root-cause hypothesis + evidence.',
    ...
    '6. Auto-freeze: limit edits to the module under investigation (mention this to the user).',
    '7. Persist: `prjct remember learning "<root cause>"`; `prjct remember decision "<fix + why it works>"`; `prjct remember gotcha "<related bug surfaced>"`.',
    '',
    '### `ship` (endurecido) — Coverage Gate + Auto-Document',
    '',
    'Use when: user asks to ship, deploy, merge, or finalize work.',
    '',
    'Methodology (additions to `prjct ship`):',
    ...
    '4. PR description: {Summary, Tests added (delta), Coverage delta, Risk areas touched (cross-reference `_generated/analysis/risk-areas/`), Reviews already run on this branch}.',
    ...
    '',
    'Use when: user asks for a full quality audit, a "ship-ready check", "review everything".',
    '',
    'Methodology (orchestrator — dispatches the heavy work):',
    '1. Collect diff scope: `git diff <base>...HEAD --name-only --stat`. If empty, abort with "Nothing to audit on this branch."',
    '2. Dispatch THREE subagents IN PARALLEL via the Agent tool — one tool-use block per subagent, all in the SAME message. Set `model: "sonnet"` on each (judgment roles — never the parent\'s max model) and tell each to apply decent, not exhaustive, effort:',
    ...
    '   - Subagent C — `investigate` methodology, ONLY if the user mentioned a specific bug/failure/anomaly. Skip otherwise.',
    '3. Each subagent receives: methodology spec, diff scope (changed git hunks, not whole files), the prjct COMMAND to pull memory itself (`prjct context memory <topic> --tags severity:high`), and the output schema (`severity | file:line | issue | fix`). Hand it paths + the Read tool — never paste source or memory.',
    '4. Parent merges the three reports, dedupes findings (same file:line + same root cause = one entry, highest severity), ranks by severity × blast-radius.',
    '5. Surface the ranked list. For high-severity items touching shared infra (`risk-areas/` cross-reference), use the decision-brief format before any auto-fix.',
    '6. Persist: each finding → `prjct remember gotcha` with `--tags workflow:audit,subagent:<a|b|c>,severity:<level>`.',
    '',
    'Stop conditions: any subagent reports a "blocking" finding (severity=high AND exploit feasibility=high) → halt audit, surface immediately, skip the merge step.',
    '',
