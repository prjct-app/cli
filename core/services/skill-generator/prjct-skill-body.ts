...
export function buildPrjctSkillBody(ctx: SkillContext): string {
  return [
    ...
    '',
    `When you actually run a heavy workflow (\`review\`, \`qa\`, \`security\`, \`investigate\`, \`audit\`, \`audit-spec\`) — or need the subagent-dispatch model policy, the parallel implementer fan-out rules, the decision-brief format, the \`prjct prefs\` protocol, the spec-pipeline stations, or the builder ethos — **read \`${PRJCT_SKILL_REFERENCE_FILE}\` in this skill directory first.** It is kept out of this file on purpose so it does not sit in your context every turn; pull it only when the task calls for it. This is the same pull-not-push rule the whole runtime follows.`,
    '',
    '## Gotchas',
    '',
