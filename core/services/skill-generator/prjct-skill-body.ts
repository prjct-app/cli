...
export function buildPrjctSkillReference(): string {
  return [
    ...
    '',
    'Intensity (opt-in, `prjct sdd`, default `off` = escalate-only as above): `advisory` nudges toward a spec for complex work; **`strict`** makes the pipeline mandatory — every `prjct task` must link a REVIEWED spec (enforced in task-service, CLI + MCP) and `ship` blocks unspecced work (`--no-spec-gate` overrides). Turn it on for projects that want spec-first always.',
    '',
    '## Builder ethos',
    '',
