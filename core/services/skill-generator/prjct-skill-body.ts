...
export function buildPrjctSkillReference(): string {
  return [
    ...
    '',
    '**Steroids:** evidence tax · finding DNA + ghost FP memory · RED/BLUE merge (contradiction auto-escalates) · blast-rank fix order · max 2 fix rounds · no `$PPID` dependency.',
    '',
    '**Ship is user-only.** Never run `prjct ship` unless the user explicitly asked to ship/publish **this turn**. When quality is incomplete and the user *does* ask to ship, the gate hard-blocks without ledger `approved` (override only with explicit consent: `prjct ship --no-spec-gate`).',
    '',
    ...
    '',
    '### `ship` (hardened) — human-only publish',
    '',
    '**STOP — never auto-ship.** Run `prjct ship` only when the user explicitly asks to ship/publish/merge/release **this turn**. Finishing quality, green tests, or a Stop-hook inject is **not** permission to ship.',
    '',
    ...
    '',
    'Loop gate: quality next card / review expected on non-trivial diffs before the human requests ship. Skip quality only for trivial intensity.',
    '',
