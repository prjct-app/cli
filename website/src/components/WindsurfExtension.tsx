...
import { BarChart3, Target, Bell, Clock, TrendingUp, Sparkles, Code2, Activity } from 'lucide-react'
...
const WindsurfExtension = () => {
  ...
  return (
    <section
      ref={containerRef}
      className="windsurf-extension-section relative overflow-hidden px-6 py-16 md:py-32"
      style={{ position: 'relative' }}
    >
      {/* Background Gradient Effects */}
      <div className="absolute inset-0 -z-10">
        <div className="absolute left-1/4 top-1/4 h-96 w-96 rounded-full bg-cat-mauve/10 blur-3xl" />
        <div className="absolute bottom-1/4 right-1/4 h-96 w-96 rounded-full bg-cat-sapphire/10 blur-3xl" />
      </div>
      ...
      <motion.div className="container mx-auto max-w-6xl" style={{ opacity }}>
        ...
        >
          ...
          <div className="mb-6 flex items-center justify-center gap-3">
            <motion.div
              ...
              className="inline-flex items-center gap-2 rounded-full border border-cat-mauve/30 bg-gradient-to-r from-purple-500/20 to-blue-500/20 px-4 py-2 backdrop-blur-sm"
            >
            ...
            <motion.div
              ...
              className="inline-flex items-center gap-2 rounded-full border border-cat-mauve/20 bg-cat-mauve/10 px-3 py-1.5"
            >
              <Clock className="h-3 w-3 animate-pulse text-cat-mauve" />
              <span className="text-xs font-medium text-cat-mauve">In Validation</span>
            </motion.div>
          </div>
        ...
        >
          {features.map((feature, index) => (
            <motion.div
              ...
              className="group relative rounded-2xl border border-border/50 bg-card/50 p-6 backdrop-blur-sm transition-all duration-300 hover:border-cat-mauve/50"
            >
              ...
              <div className="relative">
                <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-lg bg-gradient-to-br from-purple-500/20 to-blue-500/20 transition-transform duration-300 group-hover:scale-110">
                  <div className="text-cat-mauve">{feature.icon}</div>
                </div>
        ...
        <motion.div
          ...
          transition={{ duration: 0.8 }}
          viewport={{ once: true }}
          className="mx-auto mb-20 max-w-4xl rounded-2xl border border-cat-mauve/20 bg-gradient-to-br from-purple-500/5 to-blue-500/5 px-6 py-12"
        >
            ...
            {timeline.map((item, index) => (
              ...
              >
                <div className="flex items-center gap-6">
                    ...
                    <div
                      className={`h-4 w-4 rounded-full border-2 ${
                        item.status === 'completed'
                          ? 'border-cat-green bg-cat-green'
                          : item.status === 'active'
                            ? 'border-cat-mauve bg-cat-mauve shadow-lg shadow-purple-500/50'
                            : 'border-muted-foreground/30 bg-background'
                      }`}
                    >
                      {item.status === 'active' && (
                        <div className="absolute inset-0 animate-ping rounded-full bg-cat-mauve" />
                      )}
                    </div>
                    {/* Vertical Line */}
                  ...
                  {/* Content */}
                  <div className="flex-1 rounded-xl border border-border/50 bg-card/50 p-4 transition-all duration-300 hover:border-cat-mauve/30">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-right">
                        <span className="text-base font-medium text-cat-mauve">{item.date}</span>
                        {item.status === 'active' && (
                          <div className="mt-2 inline-block rounded-full bg-cat-mauve/20 px-2 py-1 text-xs text-cat-mauve">
                            Active
                          </div>
                        )}
                      </div>
