...
const WindsurfPreview = () => {
  ...
  return (
    <div className="relative">
      ...
      >
        <div className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-purple-500/20 to-blue-500/20 border border-purple-500/30 rounded-full backdrop-blur-sm">
      ...
      >
        <div className="flex gap-4">
          ...
          <div className="flex-1 rounded-lg bg-card/60 border border-border/30 overflow-hidden">
            <div className="bg-card/40 border-b border-border/20 px-4 py-2 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <div className="px-2 py-1 bg-green-500/20 text-green-500 text-xs rounded flex items-center gap-1">
            ...
            <div className="p-6 bg-background/30">
              <div className="mb-4 flex items-center justify-between">
                ...
                <motion.button
                  ...
                  className="px-3 py-1 bg-purple-500/20 text-purple-500 text-xs rounded-lg flex items-center gap-1"
                >
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  {roadmapItems.filter(i => i.status === 'in-progress').map((item, idx) => (
                    <motion.div
                      key={idx}
                      className="p-3 bg-purple-500/10 border border-purple-500/30 rounded-lg cursor-move"
                      whileHover={{ scale: 1.02, y: -2 }}
                      draggable
                    >
                      <div className="flex items-start justify-between mb-2">
                        <p className="text-xs font-medium">{item.title}</p>
                        <Edit3 className="w-3 h-3 text-purple-500 cursor-pointer" />
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="text-[10px] text-muted-foreground">{item.complexity}</span>
                        <div className="flex items-center gap-1">
                          <Bot className="w-3 h-3 text-green-500" />
                          <span className="text-[10px] text-green-500">{item.ai_confidence}%</span>
                        </div>
                      </div>
                    </motion.div>
                  ))}
                ...
                <div className="space-y-2">
                  ...
                  {roadmapItems.filter(i => i.status === 'queued').map((item, idx) => (
                    <motion.div
                      key={idx}
                      className="p-3 bg-blue-500/10 border border-blue-500/30 rounded-lg cursor-move"
                      whileHover={{ scale: 1.02, y: -2 }}
                      draggable
                    >
                      <div className="flex items-start justify-between mb-2">
                        <p className="text-xs font-medium">{item.title}</p>
                        <Edit3 className="w-3 h-3 text-blue-500 cursor-pointer" />
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="text-[10px] text-muted-foreground">{item.complexity}</span>
                        <div className="flex items-center gap-1">
          ...
          {/* PRJCT Control Panel */}
          <motion.div
            className="w-96 rounded-lg bg-gradient-to-br from-purple-500/10 via-card/80 to-blue-500/10 backdrop-blur-md border border-purple-500/20 overflow-hidden"
              ...
              <div className="space-y-3">
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full px-3 py-2 bg-background/50 border border-border/50 rounded-lg text-xs focus:outline-none focus:border-purple-500/50"
                  />
                  <motion.button
                    whileHover={{ scale: 1.05 }}
                    className="absolute right-1 top-1 px-2 py-1 bg-purple-500/20 text-purple-500 rounded text-xs"
                  >
                    Generate
                  </motion.button>
                </div>
                  ...
                  {aiSuggestions.map((suggestion, idx) => (
                    ...
                    >
                      <div className="flex items-start justify-between">
                        ...
                        <motion.button
                          whileHover={{ scale: 1.1 }}
                          className="px-2 py-1 bg-purple-500/20 text-purple-500 rounded text-[10px]"
                        >
                          Apply
                        </motion.button>
                      </div>
